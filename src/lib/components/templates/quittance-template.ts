	...
	content: [
		...
		Space(2),
		{
			text: [
				...
				)} euros), au titre du paiement du loyer et des charges du local sis `,
				{
					text: `${info.property_address}, ${info.property_postalCode} ${info.property_city}`,
					bold: true
				},
				` pour la période de location du ${dayjs(info.startDate).format('DD/MM/YYYY')} au ${dayjs(
					info.endDate
				).format('DD/MM/YYYY')} et lui en donne quittance, sous réserve de tous mes droits.`
			],
			alignment: 'justify',
			lineHeight: 1.3
		},
		...
		Text(
			`Date de paiement : ${
				info.paymentDate ? dayjs(info.paymentDate).format('DD/MM/YYYY') : 'Paiement non reçu'
			}`
		),
		Space(2),

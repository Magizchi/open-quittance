...
export const propertiesTable = mysqlTable("properties", {
  id: bigint("id", { mode: "number" }).primaryKey().autoincrement(),
  landlord_id: bigint("landlord_id", { mode: "number" })
    .notNull()
    .references(() => landlordsTable.id),
  name: varchar("name", { length: 255 }).notNull(),
  address: varchar("address", { length: 255 }).notNull(),
  city: varchar("city", { length: 255 }).notNull(),
  postalCode: varchar("postalCode", { length: 5 }).notNull(),
...
export const receiptsTable = mysqlTable("receipts", {
  id: bigint("id", { mode: "number" }).primaryKey().autoincrement(),
  rental_id: bigint("rental_id", { mode: "number" }).references(
    () => rentalsTable.id
  ),
  ...
  landlord_postalCode: varchar("landlord_postalCode", {
    length: 255,
  }).notNull(),
  tenant_id: bigint("tenant_id", { mode: "number" })
    .notNull()
  ...
  property_postalCode: varchar("property_postalCode", {
    length: 255,
  }).notNull(),

...
export const addPaymentDate = async (request: Request) => {
  ...
  try {
    await db
      ...
      .set({
        createAt: dayjs(info.paymentDate).toDate(),
        paymentDate: dayjs(info.paymentDate).toDate(),
      })
...
export const generateNewReceipts = async () => {
  ...
  let receiptOfThisMonth = (await db
    .selectDistinct({
  ...
  const rentals = (await db
    ...
    .rightJoin(
      landlordsTable,
      eq(landlordsTable.id, propertiesTable.landlord_id)
    )
    .where(
      and(
        notInArray(
          rentalsTable.id,
          receiptOfThisMonth.map((item) => item!.rental_id)
        ),
        isNull(rentalsTable.endDate)
      )
